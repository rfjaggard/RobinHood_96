...
@user_routes.route('/userNameCheck/<string:user_name>', methods=['POST'])
@login_required 
...
@user_routes.route('/userNameCheck/<string:input_email>', methods=['POST'])
@login_required 
def username_check(input_email): 
    """
    Find a user by email, return sfalse if no user is found
    """
    user = User.query.filter_by(username=input_email).first()
    if not user: 
        return jsonify({False}),200
    return jsonify(user),200